...
from cli.config import get_input, BiAlertConfig, TERRAFORM_DIR
...
class Manager:
    ...
    def cb_copy_all(self) -> None:
        """Copy all binaries from CarbonBlack Response into BiAlert
        ...
        self._enqueue(
            self._config.BiAlert_downloader_queue_name,
    ...
    def deploy(self) -> None:
        """Deploy BiAlert (equivalent to unit_test + build + apply)"""
        self.unit_test()
        self.build()
        self.apply()

    def destroy(self) -> None:
        """Teardown all of the BiAlert infrastructure"""
        os.chdir(TERRAFORM_DIR)
        ...
        if not self._config.force_destroy:
            ...
            if result == 'yes':
                print('Enabling force_destroy on the BiAlert S3 buckets...')
                subprocess.check_call([
                    'terraform', 'apply', '-auto-approve=true', '-refresh=false',
                    '-var', 'force_destroy=true',
                    '-target', 'aws_s3_bucket.BiAlert_binaries',
                    '-target', 'aws_s3_bucket.BiAlert_log_bucket'
                ])

        subprocess.call(['terraform', 'destroy'])

    def live_test(self) -> None:
        """Upload test files to BiAlert which should trigger YARA matches

        Raises:
            TestFailureError: If the live test failed (YARA matches not found).
        """
        if not live_test.run(self._config.BiAlert_s3_bucket_name,
                             self._config.BiAlert_analyzer_name,
                             self._config.BiAlert_dynamo_table_name):
            raise TestFailureError(
                '\nLive test failed! See https://BiAlert.io/troubleshooting-faq.html')
    ...
    def purge_queue(self) -> None:
        ...
        queue = boto3.resource('sqs').get_queue_by_name(
            QueueName=self._config.BiAlert_analyzer_queue_name)
        queue.purge()
    ...
    def _most_recent_manifest(bucket: boto3.resource) -> Optional[str]:
        ...
        Args:
            bucket: BiAlert S3 bucket resource
    ...
    def _s3_batch_iterator(
        ...
        for key in object_keys:
            records.append({
                's3': {
                    'bucket': {
                        'name': self._config.BiAlert_s3_bucket_name
                    },
                    'object': {
                        'key': key
                    }
    ...
    def retro_fast(self) -> None:
        ...
        bucket = boto3.resource('s3').Bucket(self._config.BiAlert_s3_bucket_name)
        ...
        self._enqueue(
            self._config.BiAlert_analyzer_queue_name,
