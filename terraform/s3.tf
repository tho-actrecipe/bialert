...
resource "aws_s3_bucket" "BiAlert_log_bucket" {
  count = var.s3_log_bucket == "" ? 1 : 0 // Create only if no pre-existing log bucket.

  bucket = format(
    "%s.BiAlert-binaries.%s.access-logs",
...
data "aws_iam_policy_document" "force_ssl_only_access" {
  ...
  statement {
    ...
    resources = [
      aws_s3_bucket.BiAlert_log_bucket[0].arn,
      "${aws_s3_bucket.BiAlert_log_bucket[0].arn}/*",
    ]
...
resource "aws_s3_bucket_policy" "force_ssl_only_access" {
  ...
  bucket = aws_s3_bucket.BiAlert_log_bucket[0].id
...
resource "aws_s3_bucket" "BiAlert_binaries" {
  bucket = "${replace(var.name_prefix, "_", ".")}.BiAlert-binaries.${var.aws_region}"
  acl    = "private"
...
data "aws_iam_policy_document" "allow_inventory" {
  statement {
    ...
    resources = ["${aws_s3_bucket.BiAlert_binaries.arn}/inventory/*"]
    ...
    condition {
      ...
      values   = [aws_s3_bucket.BiAlert_binaries.arn]
    }
  }
  ...
  statement {
    ...
    resources = [
      aws_s3_bucket.BiAlert_binaries.arn,
      "${aws_s3_bucket.BiAlert_binaries.arn}/*",
    ]
...
resource "aws_s3_bucket_policy" "allow_inventory" {
  bucket = aws_s3_bucket.BiAlert_binaries.id
...
resource "aws_s3_bucket_inventory" "binary_inventory" {
  ...
  destination {
    bucket {
      format     = "CSV"
      bucket_arn = aws_s3_bucket.BiAlert_binaries.arn
      prefix     = "inventory"
