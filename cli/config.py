"""BiAlert configuration management."""
import base64
import getpass
import os
import re
...
def get_input(prompt: str, default_value: str,
    ...
    Args:
        ...
        config: BiAlertConfig instance, if updating the underlying configuration
...
class BiAlertConfig:
    ...
    @property
    def BiAlert_analyzer_name(self) -> str:
        return '{}_BiAlert_analyzer'.format(self.name_prefix)

    @property
    def BiAlert_analyzer_queue_name(self) -> str:
        return '{}_BiAlert_analyzer_queue'.format(self.name_prefix)

    @property
    def BiAlert_downloader_queue_name(self) -> str:
        return '{}_BiAlert_downloader_queue'.format(self.name_prefix)

    @property
    def BiAlert_dynamo_table_name(self) -> str:
        return '{}_BiAlert_matches'.format(self.name_prefix)

    @property
    def BiAlert_s3_bucket_name(self) -> str:
        return '{}.BiAlert-binaries.{}'.format(
    ...
    def _encrypt_cb_api_token(self) -> None:
        ...
        response = boto3.client('kms').encrypt(
            KeyId='alias/{}_BiAlert_carbonblack_credentials'.format(self.name_prefix),
            Plaintext=api_token
        )
