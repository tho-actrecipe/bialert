...
// The production BiAlert analyzer is not analyzing binaries.
resource "aws_cloudwatch_metric_alarm" "analyzed_binaries" {
  alarm_name = "${module.BiAlert_analyzer.function_name}_no_analyzed_binaries"

  alarm_description = <<EOF
${module.BiAlert_analyzer.function_name} is not analyzing any binaries!
  - If any BiAlert Lambda function was recently deployed, roll it back via the AWS console.
...
resource "aws_cloudwatch_metric_alarm" "yara_rules" {
  alarm_name = "${module.BiAlert_analyzer.function_name}_too_few_yara_rules"

  alarm_description = <<EOF
The number of YARA rules in BiAlert is surprisingly low.
...
resource "aws_cloudwatch_metric_alarm" "dynamo_throttles" {
  alarm_name = "${aws_dynamodb_table.BiAlert_yara_matches.name}_throttles"

  alarm_description = <<EOF
Read or write requests to the BiAlert DynamoDB table are being throttled.
...
EOF
  ...
  dimensions = {
    TableName = aws_dynamodb_table.BiAlert_yara_matches.name
  }
