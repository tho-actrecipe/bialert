...
module "BiAlert_analyzer" {
  source          = "./modules/lambda"
  function_name   = "${var.name_prefix}_BiAlert_analyzer"
  ...
  environment_variables = {
    ...
    YARA_MATCHES_DYNAMO_TABLE_NAME = aws_dynamodb_table.BiAlert_yara_matches.name
...
resource "aws_lambda_event_source_mapping" "analyzer_via_sqs" {
  ...
  function_name    = module.BiAlert_analyzer.alias_arn
}

// Create the CarbonBlack downloading Lambda function.
module "BiAlert_downloader" {
  enabled         = var.enable_carbon_black_downloader ? 1 : 0
  source          = "./modules/lambda"
  function_name   = "${var.name_prefix}_BiAlert_downloader"
  description     = "Copies binaries from CarbonBlack into the BiAlert S3 bucket"
  base_policy_arn = aws_iam_policy.base_policy.arn
  ...
  environment_variables = {
    ...
    TARGET_S3_BUCKET                 = aws_s3_bucket.BiAlert_binaries.id
  }
...
resource "aws_lambda_event_source_mapping" "downloader_via_sqs" {
  ...
  function_name    = module.BiAlert_downloader.alias_arn
}
