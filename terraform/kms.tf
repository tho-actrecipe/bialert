...
resource "aws_kms_key" "sse_s3" {
  description         = "BiAlert Server-Side Encryption - S3"
...
resource "aws_kms_alias" "sse_sqs_alias" {
  name          = "alias/${var.name_prefix}_BiAlert_sse_sqs"
...
resource "aws_kms_key" "carbon_black_credentials" {
  ...
  description         = "Encrypts CarbonBlack credentials for the BiAlert downloader."
  enable_key_rotation = true
...
resource "aws_kms_alias" "encrypt_credentials_alias" {
  count         = var.enable_carbon_black_downloader ? 1 : 0
  name          = "alias/${var.name_prefix}_BiAlert_carbonblack_credentials"
