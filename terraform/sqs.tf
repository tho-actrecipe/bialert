...
resource "aws_sqs_queue" "analyzer_queue" {
  name                      = "${var.name_prefix}_BiAlert_analyzer_queue"
...
data "aws_iam_policy_document" "analyzer_queue_policy" {
  statement {
    sid = "AllowBiAlertBucketToNotifySQS"
    ...
    // Allow only the BiAlert S3 bucket to notify the SQS queue.
    condition {
      test     = "ArnEquals"
...
// Allow analyzer queue to be sent messages from the BiAlert S3 bucket.
...
resource "aws_sqs_queue" "downloader_queue" {
  ...
  name                      = "${var.name_prefix}_BiAlert_downloader_queue"
  kms_master_key_id         = aws_kms_key.sse_sqs.arn
...
resource "aws_sqs_queue" "dead_letter_queue" {
  count                     = var.enable_carbon_black_downloader ? 1 : 0
  name                      = "${var.name_prefix}_BiAlert_sqs_dead_letter_queue"
